   ...
   $this->ins_driver_u = Model_Driver::get_instance();
    ...
    public function get_user($name,$password){
        ...
        if($result == NULL || $result == FALSE){
            throw new AuthException('User with this data not found');
        return;
        }
        if(is_array($result)){
    ...
    private function package(){
        if($this->user_id){
          $arr= array(
            ...
            throw new AuthException("User id is not found");
        }
    }
    ...
    public function validate_cookie() {
        if(!$this->user_id || !$this->version || !$this->created) {
            throw new AuthException("Wrong data. Access is denied");
        }

        if($this->version != VERSION) {//соотв ли версия куки консткнте
            throw new AuthException("Incorrect version of the cook file");
        }

        if((time() - $this->created) > EXPIRATION) {
            throw new AuthException("Ended session time");
        }
